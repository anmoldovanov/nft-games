...
class Modal extends Base {
   ...
   _callHook(name, defaultValue) {
      return this.opts.hooks[name]?.(this) ?? defaultValue;
   }
   updateAriaTargets() {
   ...
   setFocusToFirstNode() {
      ...
      let elem = this.focusTrap.focusableNodes[0];
   ...
   async [ACTION_TOGGLE](s, { trigger = null } = {}) {
      ...
      if (!s && this._callHook("preventClose", !opts.closeable)) {
   ...
   _checkElementAnimation(elemName, eventName) {
      ...
      const setEmpty = () => {
         ...
         this._setTransitionClasses(elemName, []);
         promises.clear();
      };
   ...
   _setTransition(elemName, afterFrom) {
      if (!this[elemName]) return;
      let s = this.isOpen;
      this._setTransitionClasses(elemName, [s ? STATE_TRANSITION_ENTER_FROM : STATE_TRANSITION_LEAVE_FROM]);
      afterFrom?.();
      repaint(this[elemName]);
      this._setTransitionClasses(elemName, s ? [STATE_TRANSITION_ENTER_ACTIVE, STATE_TRANSITION_ENTER_TO] : [STATE_TRANSITION_LEAVE_ACTIVE, STATE_TRANSITION_LEAVE_TO]);
   }
   _setTransitionClasses(elemName, classes) {
      this.states[elemName].animations = classes;
